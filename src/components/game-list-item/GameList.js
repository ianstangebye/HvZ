    ...
    render() {
        ...
        if (this.state.games.length > 0) {
            gameComponents = this.state.games.map(game => {
                //return <GameItem game={game} key={game.game_Id} user_id={this.state.user_id} is_admin={this.state.is_admin} token={this.state.token} />
                return <GameItem game={game} key={game.game_Id} userInfo={this.state.userInfo} />
